import React from 'react';
import { AbsoluteFill, Series } from 'remotion';
import { Scene1Intro } from './scenes/Scene1Intro';
import { Scene2ValueProp } from './scenes/Scene2ValueProp';
import { Scene3Features } from './scenes/Scene3Features';
import { Scene4Outro } from './scenes/Scene4Outro';

// 210 + 240 + 360 + 150 = 960 frames = 32s @ 30fps
export const MatehiaPresentation: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: '#0c0c1a' }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@200;300;400;500;600;700;800;900&display=swap');
        * { -webkit-font-smoothing: antialiased; box-sizing: border-box; }
      `}</style>

      <Series>
        {/* 7s — OmniChat logo + scan line + arcs */}
        <Series.Sequence durationInFrames={210}>
          <Scene1Intro />
        </Series.Sequence>

        {/* 8s — Headline mot par mot + command bar + tuiles */}
        <Series.Sequence durationInFrames={240}>
          <Scene2ValueProp />
        </Series.Sequence>

        {/* 12s — 3 spotlights × 4s (Messageries / Actions / Aujourd'hui) */}
        <Series.Sequence durationInFrames={360}>
          <Scene3Features />
        </Series.Sequence>

        {/* 5s — Outro + shimmer + "Pilotez. Priorisez. Exécutez." */}
        <Series.Sequence durationInFrames={150}>
          <Scene4Outro />
        </Series.Sequence>
      </Series>
    </AbsoluteFill>
  );
};

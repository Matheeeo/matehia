...
const CANAL_CFG: Record<string, { color: string; bg: string; border: string; glyph: string }> = {
  ...
  SMS:      { color: '#fbbf24', bg: 'rgba(251,191,36,0.08)',  border: 'rgba(251,'use client';
...
// Config visuelle par canal — doit correspondre aux sources retournées par route.ts
const CANAL_CFG: Record<string, { color: string; bg: string; border: string; glyph: string }> = {
  Email:    { color: '#a78bfa', bg: 'rgba(167,139,250,0.08)', border: 'rgba(167,139,250,0.2)',  glyph: '✉' },
...
type Message = {
  id: string;              // UUID conversation Supabase → pour fetchThread
  source: string;          // 'Email' | 'SMS' | 'WhatsApp' | 'LinkedIn'
  date: string;
  sender: string;
  content: string;         // message entier (bottom sheet)
  summary: string | null;  // résumé IA (card uniquement)
    ...
    <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 18, marginBottom: 8 }}>
      ...
      <div
        ...
        style={{
          ...
          cursor: 'pointer',
          background: urgent
            ? 'linear-gradient(135deg, rgba(26,8,16,0.98) 0%, rgba(20,6,14,0.98) 100%)'
      ...
      >
        ...
        {/* CARD : résumé IA uniquement (pas le message entier) */}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: out ? 'flex-end' : 'flex-start', marginBottom: 8 }}>
      <div style={{
        ...
        <p style={{ margin: 0, fontSize: 14, lineHeight: 1.5, color: out ? '#fff' : '#e4e4e7' }}>
          {msg.body}
        </p>
      </div>
...
export default function Home() {
  ...
  // Charge le fil complet via l'UUID Supabase de la conversation
  const fetchThread = useCallback(async (convUuid: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        console.error('[thread] fetch error:', res.status, await res.text());
        return;
      }
  ...
  async function handleClick(msg: Message) {
    ...
    setSendStatus('idle');
    setSendError('');
    fetchThread(msg.id); // msg.id = UUID conversation Supabase
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (data.success) {
        setSendStatus('ok');
        const optimistic: ThreadMsg = { body: reply.trim(), direction: 'outbound', sent_at: new Date().toISOString() };
          ...
          <div style={{
            ...
            <div style={{ padding: '0 20px 12px', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 800, color: '#fafafa', letterSpacing: '-0.03em', lineHeight: 1.1 }}>
                    {selected.sender}
                  </div>
                </div>
                <button onClick={() => { setSelected(null); setThread([]); setReply(''); setSendStatus('idle'); setSendError(''); }}
            ...
            <div ref={threadRef} style={{ flex: 1, overflowY: 'auto', padding: '12px 20px', minHeight: 0 }}>
              {threadLoading ? (
                ...
                <p style={{ textAlign: 'center', color: '#3f3f46', fontSize: 13, margin: '20px 0' }}>Aucun message</p>
              ) : (
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <Pill source={message.source} small />
            {urgent && (
              <span style={{ fontSize: 10, fontWeight: 700, color: '#f87171', letterSpacing: '0.1em', textTransform: 'uppercase' }}>URGENT</span>
            )}
          </div>
          <span style={{ fontSize: 11, color: '#3f3f46', fontWeight: 500 }}>{timeAgo(message.date)}</span>
        </div>
        ...
        <p style={{
          margin: '0 0 4px 0', fontSize: 14,
          fontWeight: message.read ? 500 : 700,
          color: message.read ? '#52525b' : '#f4f4f5',
...
export default function Home() {
  ...
  async function handleSend() {
    ...
    if (!selected.conversation_id || !selected.id_destinataire) {
      setSendStatus('err');
      setSendError('Conversation introuvable');
      return;
    }
    setSending(true);
    setSendStatus('idle');
                  ...
                  <button key={s} onClick={() => setFilter(s)} style={{
                    ...
                    {count > 0 && (
                      <span style={{
                        ...
                        background: active ? 'rgba(255,255,255,0.2)' : 'rgba(124,58,237,0.15)',
            ...
            <div style={{ padding: '12px 16px 32px', flexShrink: 0, borderTop: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div style={{
                  ...
                  border: `1px solid ${reply ? 'rgba(124,58,237,0.3)' : 'rgba(255,255,255,0.06)'}`,
                ...
                <button
                  ...
                  {sending ? (
                    ...
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                      <path d="M3 8l3.5 3.5L13 4" stroke="#34d399" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  ) : (
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">

...
const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://zmimsvyxooweqefwlzyg.supabase.co',
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InptaW1zdnl4b293ZXFlZndsenlnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUxMzc0NTUsImV4cCI6MjA5MDcxMzQ1NX0.bgzxGe8stTToUYTPui-qN_jDMr7w08RRgmeNsY4KTSY'
);
        ...
        <div className="flex items-start gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-sm font-medium ${message.read ? 'text-zinc-400' : 'text-white'}`}>
              {message.sender}
              {message.priority === 'haute' && (
                <span className="ml-2 text-xs text-red-400 font-normal">urgent</span>
              )}
            </p>
...
export default function Home() {
  ...
  const fetchMessages = useCallback(async (source: string) => {
    setLoading(true);
      ...
      <main className="flex-1 overflow-y-auto px-4 pb-8 space-y-5">
        {loading ? (
          ...
          <>
            ...
            {toProcess.length > 0 && (
              <section>
                <p className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-2">À traiter</p>
                <div className="space-y-2">
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end z-50" onClick={() => setSelected(null)}>
          <div className="w-full max-w-lg mx-auto bg-zinc-900 rounded-t-2xl p-6 pb-10 space-y-3" onClick={(e) => e.stopPropagation()}>
            ...
            {selected.summary && (
              <p className="text-sm text-white/80 bg-zinc-800 rounded-lg px-3 py-2 leading-relaxed">{selected.summary}</p>
            )}
